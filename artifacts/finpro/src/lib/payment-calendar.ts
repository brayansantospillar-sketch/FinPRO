...
const isWeekdayBusinessDay = (d: Date) => d.getDay() !== 0 && d.getDay() !== 6 && !isHoliday(d);
const isSalaryCountingDay = (d: Date) => d.getDay() !== 0 && !isHoliday(d);

function moveToWeekdayBusinessDay(date: Date, direction: -1 | 1) {
  let d = new Date(date);
  do { d = addDays(d, direction); } while (!isWeekdayBusinessDay(d));
  return d;
}

export function nthSalaryBusinessDay(year: number, monthIndex: number, ordinal: number) {
  let count = 0;
  for (let day = 1; day <= 31; day++) {
    const d = new Date(year, monthIndex, day, 12);
    if (d.getMonth() !== monthIndex) break;
    if (isSalaryCountingDay(d) && ++count === ordinal) return d;
  }
  throw new Error('Não foi possível calcular o dia útil.');
}

export function resolvePaymentDate(input: {
  year: number;
  monthIndex: number;
  scheduleType: 'fixed_day' | 'business_day';
  dayOfMonth?: number | null;
  businessDayOrdinal?: number | null;
  saturdayPolicy: PaymentPolicy;
  sundayPolicy: PaymentPolicy;
  holidayPolicy: PaymentPolicy;
}) {
  let d = input.scheduleType === 'business_day'
    ? nthSalaryBusinessDay(input.year, input.monthIndex, input.businessDayOrdinal ?? 5)
    : new Date(input.year, input.monthIndex, Math.min(input.dayOfMonth ?? 1, new Date(input.year, input.monthIndex + 1, 0).getDate()), 12);

  if (isHoliday(d)) d = moveToWeekdayBusinessDay(d, input.holidayPolicy === 'previous_business_day' ? -1 : 1);
  else if (d.getDay() === 6) d = moveToWeekdayBusinessDay(d, input.saturdayPolicy === 'previous_business_day' ? -1 : 1);
  else if (d.getDay() === 0) d = moveToWeekdayBusinessDay(d, input.sundayPolicy === 'previous_business_day' ? -1 : 1);
  return d;
}
