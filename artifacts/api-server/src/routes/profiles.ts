...
router.get("/profiles", async (_req: Request, res: Response) => {
  const rows = await db.select().from(profilesTable).orderBy(asc(profilesTable.createdAt));
  res.json(rows);
});
...
router.post("/profiles", async (req: Request, res: Response) => {
  ...
  const [created] = await db.insert(profilesTable).values({ name, role: role || "Membro" }).returning();
...
router.put("/profiles/:id", async (req: Request, res: Response) => {
  ...
  const role = typeof req.body?.role === "string" ? req.body.role.trim() : "Membro";
  if (!name || name.length > 80 || role.length > 40) return void res.status(400).json({ message: "Dados do perfil inválidos." });
  const [updated] = await db.update(profilesTable).set({ name, role: role || "Membro", updatedAt: new Date() }).where(eq(profilesTable.id, String(req.params.id))).returning();
